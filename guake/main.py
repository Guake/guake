...
def main():
    ...
    parser.add_option(
        "--show", dest="show", action="store_true", default=False, help=_("Shows Guake main window")
    )

    parser.add_option(
        "--hide", dest="hide", action="store_true", default=False, help=_("Hides Guake main window")
    )
    ...
    if options.select_tab:
        ...
        if 0 <= selected < tab_count:
            ...
            sys.stderr.write(f"invalid index: {selected:d}\n")
        only_show_hide = options.show

    if options.selected_tab:
        selected = remote_object.get_selected_tab()
        sys.stdout.write(f"{selected:d}\n")
        only_show_hide = options.show
